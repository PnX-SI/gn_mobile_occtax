...
Il est cependant possible de surcoucher localement les paramètres de configuration via la création d'un fichier `settings_occtax.local.json`. Ce fichier devra être placé au même endroit que le fichier `settings_occtax.json` (dans le dossier `/Android/data/fr.geonature.occtax2/` du terminal mobile).
