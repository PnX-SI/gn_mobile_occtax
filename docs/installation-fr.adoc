...
Il est cependant possible de surcoucher localement les paramètres de configuration via la création d'un fichier `settings_occtax.local.json`. Ce fichier devra être placé au même endroit que `settings_occtax.local.json`.
