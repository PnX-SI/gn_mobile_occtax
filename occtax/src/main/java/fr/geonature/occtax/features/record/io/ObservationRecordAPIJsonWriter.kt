...
/**
 * `JsonWriter` about writing an [ObservationRecord] without taxa as `JSON` for _GeoNature_ APIs.
 *
 * @author S. Grimault
 *
 * @see TaxonRecordAPIJsonWriter
 */
class ObservationRecordAPIJsonWriter {
     ...
     *
     * @return [ObservationRecordAPIJsonWriter] fluent interface
     */
    fun setIndent(indent: String): ObservationRecordAPIJsonWriter {
        this.indent = indent
     ...
     * @see fr.geonature.occtax.features.record.io.ObservationRecordAPIJsonWriter.write(java.io.Writer, fr.geonature.occtax.features.record.domain.ObservationRecord, fr.geonature.occtax.settings.AppSettings)
     */
    fun write(
    ...
    private fun writeProperties(
        ...
        observationRecord.properties.forEach {
            ...
            when {
                ...
                propertyValue is PropertyValue.Number -> writer.name(propertyValue.code)
                    .value(propertyValue.value)

                propertyValue is PropertyValue.NumberArray -> {
                ...
                propertyValue is PropertyValue.Dataset -> writer.name(propertyValue.code)
                    .value(propertyValue.value?.id)

                propertyValue is PropertyValue.Nomenclature -> {
                ...
                propertyValue is PropertyValue.Observers -> {
                    writer.name(propertyValue.code)
                        .beginArray()
                ...
                propertyValue is PropertyValue.AdditionalFields -> {
                    writer.name(propertyValue.code)
    ...
    private fun writeDates(
        ...
        observationRecord.dates.end.run {
            writer.name(DatesRecord.DATE_MAX_KEY)
                .value(
                    if (dateSettings?.endDateSettings != null) format(
                        "yyyy-MM-dd",
                        TimeZone.getDefault()
